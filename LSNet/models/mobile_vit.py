...
class PreNormCross(nn.Module):
    ...
    def forward(self, x, x_mr, **kwargs):
        return self.fn(self.norm(x), self.norm(x_mr), **kwargs)
...
class Transformer(nn.Module):
    ...
    def forward(self, x):
        ...
        #
        #         x = x_gate * cross_attn(x, x_mr) + x  # 用branch1的信息增强branch2
        #         x = ff(x) + x
...
class MobileViTBlock(nn.Module):
    def __init__(self, dim, depth, channel, heads, kernel_size, patch_size, mlp_dim, dropout=0.):
        super().__init__()
        ...
        self.transformer = Transformer(dim, depth, heads, dim//2, mlp_dim, dropout)
    ...
    def forward(self, x):
        ...
        x = self.transformer(x)  # cross_att
...
class MobileViT(nn.Module):
    def __init__(self, image_size, dims, channels, heads, num_classes, expansion=4, kernel_size=3, patch_size=(2, 2)):
        ...
        self.mvit.append(MobileViTBlock(dims[0], L[0], channels[5], heads, kernel_size, patch_size, int(dims[0] * 2)))
        self.mvit.append(MobileViTBlock(dims[1], L[1], channels[7], heads, kernel_size, patch_size, int(dims[1] * 4)))
        self.mvit.append(MobileViTBlock(dims[2], L[2], channels[9], heads, kernel_size, patch_size, int(dims[2] * 4)))
    ...
    def forward(self, x):
        ...
        x = self.conv2(x)  # at the last, x was used for classification
