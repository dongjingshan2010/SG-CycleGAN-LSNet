...
class Domain_Discriminator(nn.Module):
    def __init__(self, input_nc):
        ...
        self.conv_layers = nn.Sequential(
            ...
            nn.Conv2d(512, 256, 4, padding=1),
            nn.InstanceNorm2d(256),
            nn.LeakyReLU(0.2, inplace=False),
        )

        # 全局平均池化
        self.global_avg_pool = nn.AdaptiveAvgPool2d(1)  # 输出形状 [batch, 256, 1, 1]

        # 全连接层，输入256维，输出2维
        self.fc = nn.Linear(256, 2)

    def forward(self, x):
        x = self.conv_layers(x)  # 输出形状: [batch, 256, H, W]
        x = self.global_avg_pool(x)  # 输出形状: [batch, 256, 1, 1]
        x = x.view(x.size(0), -1)  # 输出形状: [batch, 256]
        x = self.fc(x)  # 输出形状: [batch, 2]
        return x
...
class SharedGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, mtwg_fea, image_size, depth4vit, n_residual_blocks=7, shared_layers=3):
        ...
        shared_model += [
            ...
            nn.Conv2d(out_features*2, mtwg_fea, 7),
            nn.Tanh()
        ]
        ...
        a2b_private_model = [
            nn.ReflectionPad2d(3),
            nn.Conv2d(mtwg_fea, fix_fea, 7),
            # DRconv2d(in_channels=input_nc, out_channels=64, kernel_size=7),
            nn.InstanceNorm2d(fix_fea),
            nn.ReLU(inplace=True)
        ]

        # 下采样层
        a2b_in_features = fix_fea
        a2b_out_features = a2b_in_features * 2
        for i in range(shared_layers):
            a2b_private_model += [
                nn.Conv2d(a2b_in_features, a2b_out_features, 5, stride=2, padding=1),
        ...
        a2b_private_model += [DRconv2d(in_channels=a2b_out_features//2, out_channels=a2b_in_features, kernel_size=1), ]
        ...
        # 下采样层
        b2a_in_features = fix_fea
        b2a_out_features = b2a_in_features * 2
        for i in range(shared_layers):
            b2a_private_model += [
                nn.Conv2d(b2a_in_features, b2a_out_features, 5, stride=2, padding=1),
        ...
        b2a_private_model += [
            DRconv2d(in_channels=b2a_out_features // 2, out_channels=b2a_in_features, kernel_size=1), ]
